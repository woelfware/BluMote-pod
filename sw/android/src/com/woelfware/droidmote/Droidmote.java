...
public class Droidmote extends Activity implements OnClickListener,OnTouchListener,
													OnItemClickListener,OnItemSelectedListener 
{
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case REQUEST_MANAGE_DEVICE:
			...
			if (resultCode == Activity.RESULT_OK) {
				// re-populate the drop-down menu and set selection to the first
				// item
				// device_data.open(); // onActivityResult() is called BEFORE
				// onResume() so need this!
	...
	public void interpretResponse(byte[] response, int bytes, int index) {
		...
		case Codes.GET_VERSION:
			if (response[index] == Codes.ACK) {
				...
				// TODO need to create state machine to gather data like the
				// learn command
				try { // catch any unforseen state machine errors.....
					while (bytes > 0) {
